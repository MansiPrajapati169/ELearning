...
class OnboardingViewController: UIViewController {
    ...
    @IBAction func nextClick(_ sender: UIButton) {
        if pageControl.currentPage == collectionDataModel.count - 1 {
            Helper.showAlert(message: "next", viewController: self)
        }
    ...
    @IBAction func skipClick(_ sender: UIButton) {
        if let thirdViewController = UIStoryboard(name: Constant.authenticationStoryboard, bundle: nil).instantiateViewController(withIdentifier: Constant.authenticationIdentifier) as? AuthenticationViewController {
